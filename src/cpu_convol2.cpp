  ...
  if ((m->nt<m->batch)||(m->id<m->batch)) {
	...
	for(j=0;j<m->outc;++j,in++) {
	  ...
	  for(z=0;z<m->kz;++z) {
	    si=(i*stride)-kr2;
	    sj=(j*stride)-kc2;
	    for(i2=0;i2<m->kr;++i2,++i)
	      for(j2=0,sj=(j*stride)-kc2;j2<m->kc;++j2,im++,++j)
...
// N,K,E --> 2D Tensors
void Convol2DF(Tensor *N, Tensor *K, Tensor *E,int stride, int rpad,int cpad)
{

  // LOWERING
  Tensor *I=new Tensor(K->a*K->b,E->a*E->b);
  Tensor *Kn=new Tensor(K->a*K->b);
  Tensor *O=new Tensor(E->a*E->b);

  int i,j,k,in,im,i2,j2,b;
  int size=E->a*E->b;
  int kr2=K->a/2;
  int kc2=K->b/2;
  int si,sj;
  ...
  for(i=0;i<K->a;++i)
    for(j=0,b=i*K->b;j<K->b;++j,++b)
      Kn->ptr1(b)=K->ptr2(i,j);
  
    
  for(i=0;i<E->a;++i)
    for(j=0,in=i*E->b;j<E->b;++j,in++) {
      im=0;
      si=(i*stride)-kr2;
      sj=(j*stride)-kc2;
      for(i2=0;i2<K->a;++i2,++i)
	for(j2=0,sj=(j*stride)-kc2;j2<K->b;++j2,im++,++j)
	  if ((si<0)||(sj<0)||(si>=inr)||(sj>=inc))
	    I->ptr2(im,in)=0.0;
	  else
	    I->ptr2(im,in)=N->ptr2(si,sj);
    }
  
    
  // MAKE LOWERING CONVOLUTION
  O->ptr1=Kn->ptr1*I->ptr2;
  
    
  // RESHAPE
  for(i=0;i<E->a;++i) 
    for(j=0,k=i*E->b;j<E->b;++j,++k) 
      E->ptr2(i,j)=O->ptr1(k);
...
// N,K -> 3D Tensors
// E -> 2D Tensor
void Convol3DF(Tensor *N, Tensor *K, Tensor *E,int stride, int rpad,int cpad)
{
  int i,j,k,in,im,i2,j2,b,z;
  int size=E->a*E->b;
  int kr2=K->a/2;
  int kc2=K->b/2;
  int si,sj;
  ...
  // LOWERING
  Tensor *I=new Tensor(K->a*K->b*K->c,E->a*E->b);
  Tensor *Kn=new Tensor(K->a*K->b*K->c);
  Tensor *O=new Tensor(E->a*E->b);
  ...
  for(i=0;i<E->a;++i)
	...
	si=(i*stride)-kr2;
	sj=(j*stride)-kc2;
	for(i2=0;i2<K->b;++i2,++i)
	  for(j2=0,sj=(j*stride)-kc2;j2<K->c;++j2,im++,++j)
	    if ((si<0)||(sj<0)||(si>=inr)||(sj>=inc))
	      I->ptr2(im,in)=0.0;
	    else
	      I->ptr2(im,in)=N->ptr[z]->ptr2(si,sj);
